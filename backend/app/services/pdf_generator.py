...
def generar_supervision_pdf(contrato: dict, pago: dict, planillas: list,
    ...
    def _no_aplica(val, default="NO APLICA"):
        """Retorna el valor si es truthy, o 'NO APLICA' si está vacío."""
        if val is None:
            return default
        s = str(val).strip()
    ...
    informe = {
        ...
        "contratista": _no_aplica(contrato.get("nombre_contratista")),
        "identificacion": _no_aplica(contrato.get("identificacion")),
        "lugar_expedicion": _no_aplica(contrato.get("lugar_expedicion")),
        "telefono": _no_aplica(contrato.get("telefono")),
        "direccion": _no_aplica(contrato.get("direccion")),
        "tipo_persona": contrato.get("tipo_persona", "NATURAL"),
        "codigo_ciiu": _no_aplica(contrato.get("codigo_ciiu")),
        "supervisor": _no_aplica(contrato.get("supervisor")),
        "nivel_supervisor": contrato.get("nivel_prof_supervisor") or "UNIVERSITARIO, DESPACHO, COORDINACION",
        "interventor": _no_aplica(contrato.get("interventor")),
        "nivel_interventor": contrato.get("nivel_prof_interventor") or "N/A",
        "cdp": _no_aplica(contrato.get("no_cdp")),
        "crp": _no_aplica(contrato.get("rp")),
        "imputacion": _no_aplica(contrato.get("imputacion", "") or contrato.get("rubro", "")),
        ...
        "tiempo_adicion": _no_aplica(contrato.get("tiempo_adicion")),
        "valor_final": valor_final,
        "forma_pago": _no_aplica(contrato.get("forma_pago"), "SEGÚN CLÁUSULA DE FORMA DE PAGO DEL CONTRATO."),
        "numero_pago": pago.get("numero_pago", "_____"),
        "valor_a_pagar": valor_pagar,
        "otro_si": float(pago.get("otro_si") or 0),
        "valor_pagado": float(pago.get("valor_pagado_historico", 0)),
        "saldo_a_pagar": float(pago.get("saldo_a_pagar", 0)),
        "ibc": float(pl.get("ibc", 0)),
        "periodo_cotizado": _no_aplica(pl.get("periodo_cotizado")),
        "eps_nombre": _no_aplica(pl.get("eps_nombre")),
        "eps_valor": float(pl.get("eps_valor", 0)),
        "arl_nombre": _no_aplica(pl.get("arl_nombre")),
        "arl_valor": float(pl.get("arl_valor", 0)),
        "sena_valor": float(pl.get("sena_valor", 0)),
        "icbf_valor": float(pl.get("icbf_valor", 0)),
        "ccf_nombre": _no_aplica(pl.get("ccf_nombre")),
        "ccf_valor": float(pl.get("ccf_valor", 0)),
        "afp_nombre": _no_aplica(pl.get("afp_nombre")),
        ...
        "folios": _no_aplica(pago.get("folios")),
        "dia_firma": hoy.day,
