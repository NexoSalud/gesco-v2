...
@router.get("/by-number", response_model=ContratoOut)
async def obtener_contrato_por_numero_query(
    numero: str = Query(..., description="Número de contrato"),
    db: AsyncSession = Depends(get_db),
):
    """Obtener contrato por número vía query param (seguro contra slashes en URL)."""
    result = await db.execute(
        select(Contrato)
        .options(
            selectinload(Contrato.contratista_rel),
        ...
        .where(Contrato.numero_contrato == numero)
    )
    contrato = result.scalar_one_or_none()
    if not contrato:
        raise HTTPException(404, "Contrato no encontrado")
...
@router.post("/by-number/anular")
async def anular_contrato_por_numero_query(
    numero: str = Query(..., description="Número de contrato"),
    motivo: str = Query(..., description="Motivo de anulación"),
    db: AsyncSession = Depends(get_db),
):
    """Anular contrato por número vía query param."""
    result = await db.execute(
        select(Contrato).where(Contrato.numero_contrato == numero)
    )
    contrato = result.scalar_one_or_none()
    if not contrato:
        raise HTTPException(404, "Contrato no encontrado")
...
@router.post("/by-number/cuotas")
async def registrar_cuota_por_numero_query(
    numero: str = Query(..., description="Número de contrato"),
    accion: str = Query("sumar", description="sumar|restar|set"),
    valor: Optional[int] = Query(None),
    db: AsyncSession = Depends(get_db),
):
    """Registrar cuota por número vía query param."""
    result = await db.execute(
        select(Contrato).where(Contrato.numero_contrato == numero)
    )
    contrato = result.scalar_one_or_none()
    ...
    if accion == "sumar" and contrato.cuotas_pagadas < contrato.cuotas_total:
        ...
        contrato.cuotas_pagadas = max(0, min(valor, contrato.cuotas_total))
    await db.commit()
    return {"cuotas_pagadas": contrato.cuotas_pagadas, "cuotas_total": contrato.cuotas_total}


@router.get("/by-number/docx")
async def descargar_docx_por_numero_query(
    numero: str = Query(..., description="Número de contrato"),
    db: AsyncSession = Depends(get_db),
):
    """Descargar DOCX por número vía query param."""
    result = await db.execute(
        select(Contrato)
        .options(
            selectinload(Contrato.contratista_rel),
    ...
    filename = f"Contrato_{numero.replace('/', '_')}.docx"
    return Response(
        content=docx_bytes,
        media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
